...
public class YouTrackConnector extends AbstractRepositoryConnector {
    ...
    public static YouTrackProject getProject(TaskRepository repository,
	    String projectname) {
	if (!projectsByRepository.keySet().contains(repository)) {
	    ...
	    projectsByRepository.put(repository, new HashSet<YouTrackProject>(
		    projects));
	}

	HashSet<YouTrackProject> projects = projectsByRepository
		.get(repository);
    ...
    public static void updateProjectCustomFields(TaskRepository repository,
	    String projectname) {
	...
	if (project != null) {
	    ...
	    for (YouTrackProject repoProject : projectsByRepository
		    .get(repository)) {
    ...
    public static synchronized YouTrackClient getClient(
	...
	try {
		...
		client = YouTrackClient.createClient(repository.getUrl()
			.toString());
		boolean login = client.login(repository.getUserName(),
			repository.getPassword());
		if (!login) {
		    throw new RuntimeException("Can't login into  : "
	...
	if (taskId.contains("-")) {
	    ...
	    if (TasksUiPlugin.getTaskList().getTask(
		    taskRepository.getRepositoryUrl(), taskId) == null) {
		return null;
	    }
	    realIssueId = TasksUiPlugin.getTaskList()
		    .getTask(taskRepository.getRepositoryUrl(), taskId)
		    .getTaskKey();
	}
	...
	return new MyRunnable() {
		...
		String projectname = query
			.getAttribute(YouTrackCorePlugin.QUERY_KEY_PROJECT);
		String filter = query
			.getAttribute(YouTrackCorePlugin.QUERY_KEY_FILTER);

		int issuesCount = queryIssuesAmount(projectname, filter,
			repository);
		issues = getClient(repository)
			.getIssuesByFilter(
				getFilter(projectname, filter, repository),
				issuesCount);

		for (YouTrackIssue issue : issues) {
		    TaskData taskData = taskDataHandler.readTaskData(
			    repository, issue, monitor);
    ...
    public String getFilter(String projectname, String filter,
	    TaskRepository repository) {
    ...
    public int queryIssuesAmount(String projectname, String filter,
	    TaskRepository repository) throws CoreException {
	return getClient(repository).getNumberOfIssues(
		getFilter(projectname, filter, repository));
    }
