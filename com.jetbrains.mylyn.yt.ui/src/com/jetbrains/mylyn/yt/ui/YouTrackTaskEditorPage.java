...
public class YouTrackTaskEditorPage extends AbstractTaskEditorPage {
	...
	for (Iterator<TaskEditorPartDescriptor> it = descriptors.iterator(); it
		.hasNext();) {
	    TaskEditorPartDescriptor taskEditorPartDescriptor = it.next();
	    if (taskEditorPartDescriptor.getId().equals(ID_PART_PEOPLE)
		    || taskEditorPartDescriptor.getId().equals(
			    ID_PART_ATTRIBUTES)
		    || taskEditorPartDescriptor.getId()
			    .equals(ID_PART_COMMENTS)
	...
	ToolBarButtonContribution editButtonContribution = new ToolBarButtonContribution(
		"com.jetbrains.yt.mylyn.toolbars.update") {
	    @Override
	    protected Control createButton(Composite composite) {
		Button editButton = new Button(composite, SWT.FLAT);
		editButton.setText("Edit ");
		editButton
			.setImage(CommonImages
	...
	return new YouTrackAttributeEditorFactory(getModel(),
