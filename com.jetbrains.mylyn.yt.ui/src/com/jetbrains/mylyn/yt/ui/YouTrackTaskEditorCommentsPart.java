 ...
 * 
 * Contributors: Tasktop Technologies - initial API and implementation Jingwen Ou - comment grouping
 *******************************************************************************/
...
public class YouTrackTaskEditorCommentsPart extends AbstractTaskEditorPart {
  ...
  private class CommentGroupViewer {
    ...
    private Control createControl(final Composite parent, final FormToolkit toolkit) {
      if (renderedInSubSection) {
        ...
        if (YouTrackTaskEditorCommentsPart.this.commentAttributes.size() >= CommentGroupStrategy.MAX_CURRENT) {
  ...
  protected class CommentViewer {
    ...
    public CommentViewer(TaskAttribute commentAttribute) {
      ...
      this.taskComment =
          new TaskComment(getModel().getTaskRepository(), getModel().getTask(), commentAttribute);
    }
    ...
    public Control createControl(Composite composite, final FormToolkit toolkit) {
      ...
      if (hasIncomingChanges) {
        commentComposite.setBackground(getTaskEditorPage().getAttributeEditorToolkit()
            .getColorIncoming());
      }
    ...
    private Composite createTitle(final ExpandableComposite commentComposite,
      ...
      ImageHyperlink expandCommentHyperlink =
          createTitleHyperLink(toolkit, titleComposite, taskComment);
    ...
    private ImageHyperlink createTitleHyperLink(final FormToolkit toolkit,
        final Composite toolbarComp, final ITaskComment taskComment) {
      ...
      if (author != null
          && author.getPersonId().equalsIgnoreCase(
  ...
  private class ReplyToCommentAction extends AbstractReplyToCommentAction implements IMenuCreator {
    ...
    public ReplyToCommentAction(CommentViewer commentViewer, ITaskComment taskComment) {
      super(YouTrackTaskEditorCommentsPart.this.getTaskEditorPage(), taskComment);
  ...
  public YouTrackTaskEditorCommentsPart() {
  ...
  protected void addActionsToToolbarButton(ToolBarManager toolBarManager,
      final TaskComment taskComment, CommentViewer commentViewer) {
  ...
  protected void addActionsToToolbarTitle(ToolBarManager toolBarManager,
      final TaskComment taskComment, CommentViewer commentViewer) {}
  ...
  private TaskComment convertToTaskComment(TaskDataModel taskDataModel,
      TaskAttribute commentAttribute) {
    TaskComment taskComment =
        new TaskComment(taskDataModel.getTaskRepository(), taskDataModel.getTask(),
            commentAttribute);
    taskDataModel.getTaskData().getAttributeMapper()
        .updateTaskComment(taskComment, commentAttribute);
    return taskComment;
  }
  ...
  public void createControl(Composite parent, final FormToolkit toolkit) {
      ...
      public void menuAboutToShow(IMenuManager manager) {
        ...
        if (selection instanceof IStructuredSelection && !selection.isEmpty()) {
          ...
          if (element instanceof ITaskComment) {
            ...
            AbstractReplyToCommentAction replyAction =
    ...
    getTaskEditorPage().getEditorSite().registerContextMenu(ID_POPUP_MENU, menuManager,
  ...
  private List<CommentGroupViewer> getCommentGroupViewers() {
    ...
    List<CommentGroup> commentGroups =
        getCommentGroupStrategy().groupComments(comments, currentPersonId);
  ...
  private void initialize() {
    commentAttributes =
        getTaskData().getAttributeMapper().getAttributesByType(getTaskData(),
