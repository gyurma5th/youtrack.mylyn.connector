...
public class YouTrackAttributesPart extends AbstractTaskEditorPart {
	...
	if (attribute != null || "".equals(attribute)) {
		...
		if (editor instanceof YouTrackAttributeEditor) {
		    GridDataFactory.fillDefaults().grab(true, true)
			    .align(SWT.FILL, SWT.FILL).hint(130, 95)
	...
	for (String key : getTaskData().getRoot().getAttributes().keySet()) {
	    if (getTaskData().getRoot().getMappedAttribute(key).getMetaData()
		    .getKind() != null
		    && getTaskData().getRoot().getMappedAttribute(key)
			    .getMetaData().getKind()
			    .equals(YouTrackTaskDataHandler.CUSTOM_FIELD_KIND)) {
		addAttribute(attributesComposite, toolkit, getTaskData()
			.getRoot().getMappedAttribute(key));
	    }
	}
