...
public class YouTrackClient {
	...
	public YouTrackIssue getIssue(String id, boolean wikifyDescription) {
		if (id == null) {
			...
			YouTrackIssue issue = service.path("/issue/").path(id)
					.queryParam("wikifyDescription", String.valueOf(wikifyDescription)).accept("application/xml")
					.get(YouTrackIssue.class);
			issue.addAttachments(getAttachments(issue.getId()));
			issue.mapFields();
			return issue;
		}
	}
	...
	public List<YouTrackIssue> getIssuesInProject(String projectname, String filter, int after, int max,
			long updatedAfter) {
		try {
			List<YouTrackIssue> issues = service.path("/issue/byproject/").path(projectname)
					.queryParam("filter", filter).queryParam("after", Integer.toString(after))
					.queryParam("max", Integer.toString(max)).queryParam("updatedAfter", Long.toString(updatedAfter))
					.accept("application/xml").get(YouTrackIssuesList.class).getIssues();
	...
	public List<YouTrackProject> getProjects() {
		try {
			return service.path("/project/all").accept("application/xml").get(YouTrackProjectsList.class).getProjects();
		} catch (Exception e) {
	...
	public YouTrackProject getProject(String projectId) {
		try {
			YouTrackProject project = service.path("/admin/project/").path(projectId).accept("application/xml")
	...
	public String putNewIssue(final YouTrackIssue issue) {
		if (issue != null && issue.getProjectName() != null && issue.getSummary() != null) {
			WebResource resource = service.path("/issue").queryParam("project", issue.getProjectName())
					.queryParam("summary", issue.getSummary());
			if (issue.getDescription() != null) {
				resource = resource.queryParam("description", issue.getDescription());
			}
			ClientResponse response = checkClientResponse(resource.put(ClientResponse.class, ""), 201,
	...
	public void deleteIssue(final String issueId) {
		if (issueId != null) {
			...
			checkClientResponse(resource.delete(ClientResponse.class), 200, "Failed delete issue " + issueId);
		} else {
			throw new RuntimeException("Null issue id");
		}
	}
	...
	public void applyCommand(final String issueId, final String command) {
		if (issueId != null && command != null) {
			...
			checkClientResponse(resource.type(MediaType.APPLICATION_FORM_URLENCODED).post(ClientResponse.class, form),
	...
	public int getNumberOfIssues(String filterQuery) {
		...
		int number;
		int attemptCount = 0;
		while ((number = resource.accept("application/xml").get(XmlNumberOfIssuesParser.class).getNumber()) == -1
				&& attemptCount++ < 10) {
			try {
				Thread.sleep(1000);
	...
	public List<YouTrackIssue> getIssuesByFilter(String filterQuery, int max) {
		...
		List<YouTrackIssue> issues = resource.accept("application/xml").get(IssueCompactsList.class).getIssues();
		for (YouTrackIssue issue : issues) {
			issue.addAttachments(getAttachments(issue.getId()));
		}
		return issues;
	...
	public LinkedList<YouTrackCustomField> getProjectCustomFields(String projectname) {
		if (projectname != null) {
			try {
				return service.path("/admin/project/").path(projectname).path("/customfield").accept("application/xml")
	...
	public YouTrackCustomField getProjectCustomField(String projectname, String fieldname) {
		if (projectname != null && fieldname != null) {
			try {
				return service.path("/admin/project/").path(projectname).path("/customfield/").path(fieldname)
						.accept("application/xml").get(YouTrackCustomField.class);
			} catch (Exception e) {
	...
	public UserBundleValues getAllUserBundleValues(String bundlename) {
		...
		if (userBundleValues.getUserGroupValues() != null) {
			for (UserGroupValue groupValue : userBundleValues.getUserGroupValues()) {
				try {
					int start = 0;
					LinkedList<UserValue> users = getUsersListInGroup(groupValue.getValue(), start).getUsers();
					while (users != null) {
	...
	public GroupUsersList getUsersListInGroup(String groupname, int start) {
		return service.path("/admin/user").queryParam("group", groupname).queryParam("start", String.valueOf(start))
				.accept("application/xml").get(GroupUsersList.class);
	}
	...
	public void addComment(final String issueId, final String comment) {
		if (issueId != null && comment != null) {
			...
			checkClientResponse(service.path("/issue/").path(issueId).path("/execute")
	...
	public String[] intellisenseFullOptions(String filter, int caret) {
		return service.path("/issue/intellisense").queryParam("filter", filter)
				.queryParam("caret", String.valueOf(caret)).accept("application/xml").get(IntellisenseValues.class)
				.getFullOptions();
	}
	...
	public IntellisenseValues intellisenseCommandValues(String command, int caret, String issueId) {
		return service.path("/issue/").path(issueId).path("/execute/intellisense").queryParam("command", command)
				.queryParam("caret", String.valueOf(caret)).accept("application/xml").get(IntellisenseValues.class)
				.getIntellisenseValues();
	}
	...
	public LinkedList<IntellisenseItem> intellisenseCommandItems(String command, int caret, String issueId) {
		return service.path("/issue/").path(issueId).path("/execute/intellisense").queryParam("command", command)
				.queryParam("caret", String.valueOf(caret)).accept("application/xml").get(IntellisenseValues.class)
				.getIntellisenseItems();
	}
	...
	public String[] getAllLinkTypeCommands() {
		return service.path("/admin/issueLinkType").accept("application/xml").get(IssueLinkTypesList.class)
				.getAllLinkTypeCommands();
	}
	...
	public void updateIssueSummaryAndDescription(final String issueId, final String newSummary,
		...
		checkClientResponse(resource.type(MediaType.APPLICATION_FORM_URLENCODED).post(ClientResponse.class, form), 200,
	...
	public boolean needUpdateDescription(YouTrackIssue oldIssue, YouTrackIssue newIssue) {
		return newIssue.getDescription() != null
				&& (oldIssue.getDescription() == null || !oldIssue.getDescription().equals(newIssue.getDescription()));
	}

	public boolean needUpdateCustomField(YouTrackIssue oldIssue, YouTrackIssue newIssue, String customFieldName) {
		...
		if (customFieldInfo.isSingle()) {
			return newIssue.getSingleCustomFieldValue(customFieldName) != null
					&& (oldIssue.getSingleCustomFieldValue(customFieldName) == null
							|| !oldIssue.getSingleCustomFieldValue(customFieldName)
									.equals(newIssue.getSingleCustomFieldValue(customFieldName)));
		} else {
	...
	public void updateIssue(String oldIssueId, YouTrackIssue newIssue) {
		...
		if (oldIssueId != null) {
			...
			for (String customFieldName : newIssue.getCustomFieldsValues().keySet()) {
				...
				if (YouTrackCustomFieldType.getTypeByName(customFieldInfo.getType())
	...
	public YouTrackTimeSettings getTimeTrackingSettings() {
		return service.path("/admin/timetracking").accept("application/xml").get(YouTrackTimeSettings.class);
	}

	private List<IssueAttachment> getAttachments(String issueId) {
		ArrayList<IssueAttachment> retVal = new ArrayList<IssueAttachment>();

		WebResource resource = service.path("/issue/").path(issueId).path("/attachment");
		LinkedList<YouTrackFileUrl> fileUrls = resource.accept("application/xml").get(YouTrackFileUrlList.class)
				.getFileUrls();
		for (YouTrackFileUrl url : fileUrls) {
			String[] nameParts = url.getName().split("__");
			String name = nameParts[nameParts.length-1];
			String description = "";
			if (nameParts.length > 1) {
				description = org.apache.commons.lang.StringUtils.join(Arrays.copyOfRange(nameParts, 0, nameParts.length - 1), "__").replace("_", "/");
			}
			Date created = null;
			try {
				long x = Long.parseLong(url.getCreated());
				created = new Date(x);
			} catch (NumberFormatException e) {
				// do not set date
			}
			retVal.add(new IssueAttachment(name, url.url, description, url.getId(), url.getAuthorLogin(), created ));
		}
		return retVal;
	}

	public void addAttachment(String issueId, String contentType, String name, long length, String description,
			InputStream attachment, IProgressMonitor monitor) {
		WebResource resource = service.path("/issue/").path(issueId).path("attachment");

		String[] mediaTypeElement = contentType.split("/");
		StreamDataBodyPart attachmentBody;
		if (mediaTypeElement.length == 2) {
			attachmentBody = new StreamDataBodyPart(name, attachment, name, new MediaType(mediaTypeElement[0], mediaTypeElement[1]));
		} else {
			attachmentBody = new StreamDataBodyPart(name, attachment, name);
		}
	...
	public InputStream getAttachment(String url) throws URISyntaxException {
		WebResource resource = service.uri(new URI(url));
		ClientResponse response = resource.get(ClientResponse.class);
		checkClientResponse(response, 200, "Failed to download attachment ");
